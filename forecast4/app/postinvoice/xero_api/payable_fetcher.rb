module XeroApi
  class PayableFetcher
    ...
    def find_existing_invoices(invoice)
      seller.payables.where(invoice_xero_id: invoice.invoice_id)
    end

    def get_invoices_count
      xero_client.Invoice.all(where: 'Type == "ACCPAY"').count
    end

    def xero_invoices(page=1)
      @invoices +=
        xero_client.Invoice.all(page: page, where: 'Type == "ACCPAY"')
